...
(function () {
  ...
  async function loadUserPlans() {
    ...
    try {
      ...
      // Display plans with clean professional design
      plansGrid.innerHTML = plans.map((plan, index) => `
        <div class="plan-card-pro" style="background: white; border: 1px solid #e5e7eb; border-radius: 12px; padding: 20px; transition: all 0.2s ease; box-shadow: 0 1px 2px rgba(0, 0, 0, 0.05); animation: planCardSlideIn 0.3s ease ${index * 0.05}s both;" onmouseover="this.style.borderColor='#d1d5db'; this.style.boxShadow='0 4px 6px rgba(0, 0, 0, 0.1)'; this.style.transform='translateY(-2px)';" onmouseout="this.style.borderColor='#e5e7eb'; this.style.boxShadow='0 1px 2px rgba(0, 0, 0, 0.05)'; this.style.transform='translateY(0)';">
          <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 16px;">
            <div style="flex: 1; min-width: 0;">
              <h3 style="font-size: 18px; font-weight: 700; color: #111827; margin: 0 0 6px 0; letter-spacing: -0.01em; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${plan.destination || plan.destinations || plan.form_data?.destination || 'Your Trip'}</h3>
              <p style="font-size: 13px; color: #6b7280; margin: 0; font-weight: 500;">${formatPlanDate(plan.created_at)}</p>
            </div>
            <span style="padding: 4px 10px; border-radius: 6px; font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.03em; flex-shrink: 0; margin-left: 12px; ${plan.status === 'completed' ? 'background: #ecfdf5; color: #059669;' : 'background: #fef3c7; color: #d97706;'}">${plan.status === 'completed' ? 'Ready' : 'Processing'}</span>
          </div>
          <div style="display: flex; gap: 12px; margin-bottom: 16px; flex-wrap: wrap;">
            ${plan.days ? `<div style="display: flex; align-items: center; gap: 6px; padding: 6px 10px; background: #f3f4f6; border-radius: 6px; font-size: 13px; font-weight: 600; color: #374151;"><svg width="14" height="14" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z"/></svg>${plan.days} days</div>` : ''}
            ${plan.budget ? `<div style="display: flex; align-items: center; gap: 6px; padding: 6px 10px; background: #f3f4f6; border-radius: 6px; font-size: 13px; font-weight: 600; color: #374151;"><svg width="14" height="14" fill="currentColor" viewBox="0 0 20 20"><path d="M8.433 7.418c.155-.103.346-.196.567-.267v1.698a2.305 2.305 0 01-.567-.267C8.07 8.34 8 8.114 8 8c0-.114.07-.34.433-.582zM11 12.849v-1.698c.22.071.412.164.567.267.364.243.433.468.433.582 0 .114-.07.34-.433.582a2.305 2.305 0 01-.567.267z"/><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-13a1 1 0 10-2 0v.092a4.535 4.535 0 00-1.676.662C6.602 6.234 6 7.009 6 8c0 .99.602 1.765 1.324 2.246.48.32 1.054.545 1.676.662v1.941c-.391-.127-.68-.317-.843-.504a1 1 0 10-1.51 1.31c.562.649 1.413 1.076 2.353 1.253V15a1 1 0 102 0v-.092a4.535 4.535 0 001.676-.662C13.398 13.766 14 12.991 14 12c0-.99-.602-1.765-1.324-2.246A4.535 4.535 0 0011 9.092V7.151c.391.127.68.317.843.504a1 1 0 101.511-1.31c-.563-.649-1.413-1.076-2.354-1.253V5z"/></svg>$${plan.budget}</div>` : ''}
          </div>
          <button onclick="event.stopPropagation(); viewPlan('${plan.id}')" style="width: 100%; padding: 12px; background: #4f46e5; border: none; border-radius: 8px; font-size: 14px; font-weight: 600; color: white; cursor: pointer; transition: all 0.2s ease;" onmouseover="this.style.background='#4338ca';" onmouseout="this.style.background='#4f46e5';">
            View Plan
          </button>
        </div>
      `).join('');
