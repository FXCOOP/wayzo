...
(function () {
  ...
  async function loadUserPlans() {
    ...
    try {
      ...
      // Display plans with better visual design
      plansGrid.innerHTML = plans.map((plan, index) => `
        <div class="plan-card-modern" style="background: white; border: 1px solid #e2e8f0; border-radius: 16px; padding: 24px; transition: all 0.3s ease; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.06); position: relative; overflow: hidden;" onmouseover="this.style.borderColor='#8b5cf6'; this.style.boxShadow='0 8px 20px rgba(139, 92, 246, 0.15)'; this.style.transform='translateY(-4px)';" onmouseout="this.style.borderColor='#e2e8f0'; this.style.boxShadow='0 2px 8px rgba(0, 0, 0, 0.06)'; this.style.transform='translateY(0)';">
          <div style="position: absolute; top: 0; right: 0; width: 100px; height: 100px; background: linear-gradient(135deg, rgba(139, 92, 246, 0.05) 0%, rgba(99, 102, 241, 0.05) 100%); border-radius: 0 16px 0 100%;"></div>
          <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 18px; position: relative;">
            <div style="flex: 1; min-width: 0;">
              <h3 style="font-size: 19px; font-weight: 700; color: #111827; margin: 0 0 8px 0; letter-spacing: -0.02em;">${plan.destination || plan.destinations || plan.form_data?.destination || 'Your Trip'}</h3>
              <p style="font-size: 14px; color: #64748b; margin: 0; font-weight: 500;">${formatPlanDate(plan.created_at)}</p>
            </div>
            <span style="padding: 6px 12px; border-radius: 8px; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; flex-shrink: 0; margin-left: 16px; ${plan.status === 'completed' ? 'background: linear-gradient(135deg, #d1fae5 0%, #a7f3d0 100%); color: #047857;' : 'background: linear-gradient(135deg, #fef3c7 0%, #fde68a 100%); color: #b45309;'}">${plan.status === 'completed' ? '✓ Ready' : '⏳ Processing'}</span>
          </div>
          <div style="display: flex; gap: 12px; margin-bottom: 20px; flex-wrap: wrap; position: relative;">
            ${plan.days ? `<div style="display: flex; align-items: center; gap: 8px; padding: 8px 14px; background: linear-gradient(135deg, #f5f3ff 0%, #ede9fe 100%); border-radius: 10px; font-size: 14px; font-weight: 600; color: #7c3aed;"><svg width="16" height="16" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z"/></svg>${plan.days} days</div>` : ''}
            ${plan.budget ? `<div style="display: flex; align-items: center; gap: 8px; padding: 8px 14px; background: linear-gradient(135deg, #f5f3ff 0%, #ede9fe 100%); border-radius: 10px; font-size: 14px; font-weight: 600; color: #7c3aed;"><svg width="16" height="16" fill="currentColor" viewBox="0 0 20 20"><path d="M8.433 7.418c.155-.103.346-.196.567-.267v1.698a2.305 2.305 0 01-.567-.267C8.07 8.34 8 8.114 8 8c0-.114.07-.34.433-.582zM11 12.849v-1.698c.22.071.412.164.567.267.364.243.433.468.433.582 0 .114-.07.34-.433.582a2.305 2.305 0 01-.567.267z"/><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-13a1 1 0 10-2 0v.092a4.535 4.535 0 00-1.676.662C6.602 6.234 6 7.009 6 8c0 .99.602 1.765 1.324 2.246.48.32 1.054.545 1.676.662v1.941c-.391-.127-.68-.317-.843-.504a1 1 0 10-1.51 1.31c.562.649 1.413 1.076 2.353 1.253V15a1 1 0 102 0v-.092a4.535 4.535 0 001.676-.662C13.398 13.766 14 12.991 14 12c0-.99-.602-1.765-1.324-2.246A4.535 4.535 0 0011 9.092V7.151c.391.127.68.317.843.504a1 1 0 101.511-1.31c-.563-.649-1.413-1.076-2.354-1.253V5z"/></svg>$${plan.budget}</div>` : ''}
          </div>
          <button onclick="event.stopPropagation(); viewPlan('${plan.id}')" style="width: 100%; padding: 14px; background: linear-gradient(135deg, #8b5cf6 0%, #6366f1 100%); border: none; border-radius: 10px; font-size: 15px; font-weight: 700; color: white; cursor: pointer; transition: all 0.3s ease; box-shadow: 0 4px 12px rgba(139, 92, 246, 0.3); position: relative;" onmouseover="this.style.background='linear-gradient(135deg, #7c3aed 0%, #4f46e5 100%)'; this.style.boxShadow='0 6px 16px rgba(139, 92, 246, 0.4)'; this.style.transform='translateY(-2px)';" onmouseout="this.style.background='linear-gradient(135deg, #8b5cf6 0%, #6366f1 100%)'; this.style.boxShadow='0 4px 12px rgba(139, 92, 246, 0.3)'; this.style.transform='translateY(0)';">
            View Plan →
          </button>
        </div>
      `).join('');
