...
(function () {
  ...
  fullPlanBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      // FIXED: Always show full plan on staging (no paywall or signup)
      // Don't strip widgets in full plan view - they should be visible
      previewEl.innerHTML = result.html || '';
      console.log('✅ Full plan displayed - staging bypass active, no paywall');
  ...
  const init = () => {
    ...
    // FIXED: Ensure login not shown on staging (auth bypass)
    if (loginBtn) {
      loginBtn.classList.add('hidden');
      console.log('🔓 Login button hidden - staging auth bypass active');
    }
  ...
  // FIXED: Authentication System (staging bypass - September 6 golden period)
  let isAuthenticated = true; // Always authenticated on staging
  let currentUser = { 
    name: 'Staging User', 
    email: 'staging@wayzo.com', 
    avatar: '/frontend/assets/default-avatar.svg',
    isStaging: true // Flag for staging environment
  };
