...
Create the most amazing, detailed, and useful trip plan possible!`;
  ...
  try {
    ...
    // Add Google Map Preview at the end if not present
    if (!md.includes('## Google Map Preview')) {
      const places = destination.split(',')[0].trim(); // Use main destination
      md += `\n\n---\n\n## Google Map Preview\n\n[Open Map](https://maps.google.com/maps?q=${encodeURIComponent(places)}+attractions+restaurants+hotels) - View all attractions, restaurants, and accommodations on one map.\n`;
    }
...
app.post('/api/plan', async (req, res) => {
  ...
  try {
    ...
    // Add Google Map Preview at the end of the report
    const markdownWithMap = markdown.includes('## Google Map Preview') ? markdown : 
      markdown + `\n\n---\n\n## Google Map Preview\n\n[Open Map](https://maps.google.com/maps?q=${encodeURIComponent(payload.destination)}+attractions+restaurants+hotels) - View all attractions, restaurants, and accommodations on one map.\n`;
