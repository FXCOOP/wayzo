...
Create the most amazing, detailed, and useful trip plan possible!`;
  ...
  try {
    ...
    if (!md.includes('## Google Map Preview')) {
      const berlinPoints = "Brandenburg+Gate+Berlin,Berlin+Wall+Memorial+Berlin,Museum+Island+Berlin,Reichstag+Building+Berlin,East+Side+Gallery+Berlin,Checkpoint+Charlie+Berlin,Potsdamer+Platz+Berlin,Café+Einstein+Stammhaus+Berlin,Kreuzburger+Berlin,Mustafa%27s+Gemüse+Kebap+Berlin,Markthalle+Neun+Berlin,Curry+36+Berlin,Lokal+Berlin,Generator+Berlin+Mitte,MEININGER+Hotel+Berlin+Hauptbahnhof,Motel+One+Berlin-Tiergarten";
