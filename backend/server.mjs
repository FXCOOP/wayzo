...
const WAYZO_VERSION = 'pro-report-2025-08-19-v6';
...
// also mount at /frontend so /frontend/style.css & /frontend/app.js resolve
...
// and keep root mount so /style.css or /app.js also work
...
// ---------- Affiliates (supports both env name styles) ----------
const AFF = {
...
async function generateWithOpenAI(payload) {
  ...
  const sys = [
    ...
    'Every recommendation must include a short what/why/value sentence.',
    ...
    'No real URLs. No HTML. Follow the requested section order exactly.',
  ].join(' ');
  ...
  const user = [
    ...
    '- Meals: Breakfast <Place>. [Reviews](reviews:<Place>) | [Map](map:<Place>); Lunch <...>; Dinner <...>.',
  ...
  if (openaiEnabled) {
    const resp = await openai.chat.completions.create({
      ...
      temperature: 0.5,
      max_tokens: Number(process.env.WAYZO_MAX_TOKENS || 2000),
      messages: [{ role: 'system', content: sys }, { role: 'user', content: user }],
    });
    md = resp.choices?.[0]?.message?.content?.trim() || '';
  }

  // Guard: if output is missing sections, use local PRO fallback
  if (!md || !looksComplete(md)) {
    md = localPlanMarkdown(payload);
  }

  // Convert token links to real URLs
  return linkifyTokens(md, destination || '');
}
