  ...
  try {
    ...
    if (tripOverviewH2) {
      ...
      weatherSection.innerHTML = `
        ...
        <table class="budget-table" style="border-collapse: collapse; border: 1px solid black; width: 100%;">
          ...
          <tbody>
            ...
            <tr><td style="border: 1px solid black; padding: 5px;">Sep 27</td><td style="border: 1px solid black; padding: 5px;">16°-22°</td><td style="border: 1px solid black; padding: 5px;">10%</td><td style="border: 1px solid black; padding: 5px;">Light clouds, great for sightseeing</td></tr>
      ...
      if (nextH2) {
        ...
        console.log('Weather forecast section injected successfully');
      }
    }
...
// Extract places from HTML and build Google Map
function extractPlacesAndBuildMap(html, destination) {
  try {
    ...
    const sections = doc.querySelectorAll('h2, h3, li, p');
    sections.forEach(element => {
      const text = element.textContent;
      
      // Match patterns like "Villa Carlotta at Via Regina 2" or "Hotel Name at Address"
      const matches = text.match(/([A-Z][^(]+?)\s+at\s+([^,]+)/g);
      if (matches) {
        matches.forEach(match => {
          const cleanMatch = match.replace(/\s+at\s+/, '+').replace(/\s+/g, '+');
          places.add(cleanMatch + '+' + destinationName);
        });
      }
    });
    
    // Build Google Map URL
    const placesList = Array.from(places).slice(0, 30); // Limit to 30 places
    ...
    const updatedHtml = html + mapSection;
    console.log(`Google Map added (fallback) with ${placesList.length} places`);
    return updatedHtml;
