...
const AFF = {
  ...
  tripadvisorAid: process.env.TRIPADVISOR_AID || '',
};
export function affiliatesFor(dest = '') {
  ...
  const tripadvisorAidParam = AFF.tripadvisorAid ? `&#x26;aff=${AFF.tripadvisorAid}` : '';
  return {
    ...
    reviews:   (term) => `https://www.tripadvisor.com/Search?q=${encodeURIComponent(term || dest)}${tripadvisorAidParam}`,
